...
public class ResourceTest : MonoBehaviour
{
    ...
    {
        TestSerilize testSerilize;
        using (FileStream fs = new FileStream(Application.dataPath + "/bear.xml", FileMode.Create))
        {
            XmlSerializer xs = new XmlSerializer(typeof(TestSerilize));
            testSerilize = (TestSerilize)xs.Deserialize(fs);
            fs.Close();
        }
        return testSerilize;
    }
