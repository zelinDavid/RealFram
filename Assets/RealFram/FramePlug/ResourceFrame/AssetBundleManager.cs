...
{
    //assetbundle配置表， 里面说明了需要加载哪些包
    protected string m_ABConfigABName = "assetbundleconfig";
    //资源关系依赖配表，可以根据crc来找到对应资源块 // resouceItem,  包含了资源的所有相关信息， 并且里面有一个实例化对象
    protected Dictionary<uint, ResouceItem> m_ResouceItemDic = new Dictionary<uint, ResouceItem>();
    //储存已加载的AB包，key为crc //AssetBundleItem 从内存加载对应资源的ab对象
...
// ResourceItem管理: 每一个实例对象对应一个ResourceItem， ResourceItem也可以不实例化对象；
// ResourceItem 也可以不实例化对象，只保存ab包相关信息
// 当ResourceItem引用计数小于1的时候，将其重置回对象池
